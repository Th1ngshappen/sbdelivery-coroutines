...
class DishesRepository @Inject constructor(
    ...
    override suspend fun findSuggestions(query: String): Map<String, Int> {
        return if (query.isEmpty()) emptyMap()
            ...
            val dishes = dishesDao.findDishesFrom(query)
            val regex = Regex("[^\\s.,!?\"-]*$query[^\\s.,!?\"-]*", RegexOption.IGNORE_CASE)
            dishes.flatMap { dish -> regex.findAll(dish.name) }
                .map { it.value }
                .groupBy { it }
                .map { it.key.lowercase() to it.value.size }
                .toMap()
        }
    }
