...
class DishRepository @Inject constructor(
    ...
    override suspend fun addToCart(id: String, count: Int) {
        val currentCount = cartDao.dishCount(id) ?: 0
        if (currentCount == 0) cartDao.addItem(CartItemPersist(dishId = id, count = count))
        else cartDao.updateItemCount(id, currentCount + count)
    }
